...
// 文字を表示するためのHTML要素を取得・・jQueryでは構文内で直接取得するので定数の設定は不要
// const untypedfield = $('#untyped').text(); //未入力の文字列、()内の'untyped'はHMTLからとってきたid要素
// const typedfield = $('#typed').text(); //入力済み文字列、()内の'untyped'はHMTLからとってきたid要素
// const wrap = $('#wrap').text();
// const start = $('#start').text();
// const count = $('#count').text();
// const typeCount = $('#typecount').text();
...
const createText = () => {
  // 正タイプした文字列をクリア
  typed = ''; //変数typedに空欄を設定 
  //HTML内のtypefieldに上記で空白にしたtypedを差し込むことでブラウザ内のtypefieldエリアをクリアすることができる
    $('#typed').text(typed);
  ...
  // 変数「untyped」に「textLists」の配列の値をランダムに代入されると定義付け
  untyped = textLists[random];
  // HTML要素のid=untypedに変数untypedで設定した内容をuntyped変数の値に挿入し、ブラウザに表示する
...
// 関数２：キー入力の判定
  ...
  function keyPress(e) {
    ...
    if (e.key !== untyped.substring(0, 1)) {
      ...
      $('#wrap').addClass('mistyped'); //コード内で誤り.mistypedと最初書いてましたが、これを書くとセレクタになってしまう
      // 100ミリ秒後に背景色を元に戻す
      setTimeout(() => {
        // 100ミリ秒後の処理内容
...
const gameOver = (id) => {
  ...
  clearInterval(id);
    // 正タイプした文字列をクリア
    typed = '';
    // 空白にした変数typedをHTML要素の'#typed'に差し込む
    $('#typed').text(typed);
    // 空白のHTML要素の'#typed'に「タイムアップ！」を差し込み、HTMLに表示
    $('#untyped').text(`タイムアップ！`);
...
const timer = () => {
  ...
  // setInterval　※関数・メソッド: 処理の間隔を設定する
